...
function replaceMultiple(text: string, map: ConversionMap, useRegex: boolean): string {
    let result = text;
    for (const key in map) {
        if (Object.prototype.hasOwnProperty.call(map, key)) {
...
const IsBanglaPostKar = (n: string): boolean => ["া", "ো", "ৌ", "ৗ", "ু", "ূ", "ী", "ৃ"].includes(n);
const IsBanglaKar = (n: string): boolean => IsBanglaPreKar(n) || IsBanglaPostKar(n);
...
function ReArrangeUnicodeConvertedText(n: string): string {
    ...
    for (let t = 0; t < n.length; t++) {
        ...
        if (t < n.length - 1 && IsBanglaPreKar(n.charAt(t)) && !IsSpace(n.charAt(t + 1))) {
            ...
            let h = 0;
            if (n.charAt(t) === "ে" && n.charAt(t + r + 1) === "া") { u += "ো"; h = 1; }
            else if (n.charAt(t) === "ে" && n.charAt(t + r + 1) === "ৗ") { u += "ৌ"; h = 1; }
            else { u += n.charAt(t); }
